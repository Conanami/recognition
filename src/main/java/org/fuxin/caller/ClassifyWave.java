...
public class ClassifyWave implements IClassifyWave{
	...
	@Override
	public WaveFileResult Filter(String phonenumber, File file, ArrayList<StandFile> standlist) {
		
		WaveFileReader reader = new WaveFileReader(file.getPath());  
		WaveFileResult rtwfr = new WaveFileResult(file.getPath());
		//根据电话号码决定运营商
        rtwfr.setPhonenumber(phonenumber);
        Operator operator = OpMgr.findOp(phonenumber.substring(0, 3));  
